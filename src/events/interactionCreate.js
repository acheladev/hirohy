...
export default (client) => {
  client.ws.on("interactionCreate", (interaction) => {
    ...
    if (
      ...
      !interaction.member.permissions.has(command.data.permission)
    )
      return interaction.reply({
        embeds: [
          new EmbedBuilder().setDescription(
            `Bu komutu kullanabilmek için \`${command.data.permission}\` yetkisine sahip olmanız gerekli.`
          ),
        ],
        flags: "Ephemeral",
      });
    ...
    if (cooldown)
      return interaction.reply({
        embeds: [
          new EmbedBuilder().setDescription(
            `Komutu tekrar kullanabilmek için \`${cooldown}\` saniye daha bekleyiniz.`
          ),
        ],
        flags: "Ephemeral",
      });
    ...
    try {
      ...
      interaction.reply({
        embeds: [new EmbedBuilder().setDescription("Bir hata oluştu!")],
        flags: "Ephemeral",
      });
      console.log(e);
    }
